...
**✨ 新增 {区块名.结果Sheet名} 组合精确引用语法**

区块名不唯一时，用 `.` 分隔同时指定区块名和结果Sheet名，精确引用某个任务的结果。

| 改动 | 说明 |
|------|------|
| **组合语法支持** | `{区块名.结果Sheet名}` 同时匹配区块名和结果Sheet名，精确指定任务 |
| **block_results 组合 key** | 存入时额外用 `(区块名, 结果Sheet名)` tuple 作为 key |
| **重复警告更新** | 区块名重复时提示用 `{区块名.结果Sheet名}` 引用 |

**引用语法（按精确度）**：
1. `{区块名.结果Sheet名}` → 精确匹配（推荐，区块名不唯一时使用）
2. `{区块名}` → 区块名唯一时使用
...
| 3 | `{汇总.地区汇总}` | 地区 | 销售额_求和 | 二次分析 | 分析 |
| 4 | `{汇总.产品汇总}` | 产品 | 销量_求和 | 二次分析2 | 分析 |

> 任务1/2区块名都是"汇总"，后续任务3/4用 `{区块名.结果Sheet名}` 精确引用
...
2. 后续任务数据源用 `{区块名}` 或 `{结果Sheet名}` 引用（按唯一性选择，见下）
