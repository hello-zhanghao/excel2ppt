...
在「聚合方式」列用 `|` 附加 Excel 数字格式，控制结果单元格的显示格式（原始数值不变，只改显示）。

**语法**（复用 join 的 `|`，两者正交：join 的 `|` 后是分隔符，其他聚合的 `|` 后是数字格式）：

| 写法 | 含义 | 示例显示 |
|------|------|---------|
| `sum\|#,##0` | 千分位整数 | `4,800` |
| `avg\|0.00` | 2位小数 | `1234.56` |
| `pct\|0.00%` | 2位小数百分比（覆盖默认 0.0%） | `37.65%` |
| `sum\|0.0%` | 百分比（数值 ×100 显示） | `37.6%` |
| `去重拼接\|、` | join 分隔符（与格式正交，不冲突） | `产品A、产品B` |

**多聚合示例**：`sum|#,##0,sum|0.00`（逗号分隔多聚合，格式内的逗号如 `#,##0` 自动保护不被切断）

**优先级**：自定义格式 > pct 默认（`0.0%`）> 普通默认（`0.00`）

> 原始数值完整写入 Excel（如 `0.3765`），`number_format` 控制显示（如 `0.00%` 显示为 `37.65%`），不影响后续 PPT/HTML 读取。
