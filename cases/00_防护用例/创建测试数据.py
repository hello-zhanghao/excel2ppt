...
def create_data_file():
    ...
    # 报告图片列：每行指向同一张测试图片，供透视后按地区取路径（演示 {{图片:区块名.列名.行值}}）
...
def create_config_file():
    ...
    pivot_rows = [
        ...
        # 值计算末尾故意加换行符，验证换行符处理（v2.18.12 修复）
        ...
        # 任务17: 散点图数据（城市级别销售额与销量，X轴Y轴都是数值）
        ...
        # 任务21: 多列组合计算——值映射与原始字段名完全不同（无子串关系）
        # 验证值计算在列名完全不同的情况下仍能正确找到对应列
        [21, "测试数据.xlsx", "销售明细", "地区", "", "销售额,销量",
         "sum,sum", "地区均价分析", "", "金额,数量",
         "", "金额/数量=均价(万元/个)", "是", "", "多列组合_异名映射"],

        # 任务22: 地区图片路径（字符串列，max 聚合保留路径值，供模板 {{图片:区块名.列名.行值}} 取用）
    ...
    ppt_rows = [
        ...
        # 页12: 章节分隔 - 高级图表
        [12, "章节", "三、高级图表与多布局测试|散点图 · 环形图 · 面积图 · 组合图 · 多图布局", "",
         "", "", "", "",
         "", "", "", "",
         "是", ""],

        # 页13: 2图上下（面积图+柱状图）
        [13, "内容", "季度趋势与地区对比", "2图上下",
         "area", "季度趋势", "季度", "总销售额",
         "季度销售额趋势(面积图)", "", "", "透视结果",
         "是", ""],
        ["", "", "", "",
        ...
        # 页19: 组合图（柱+折线）
        [19, "内容", "地区销售组合图(柱+折线)", "1图",
         "combo", "组合图数据", "地区", "总销售额",
         "销售额(柱) + 销量(折)", "", "销售额最高: {max_cat} ({max_val})", "透视结果",
         "是", ""],

        # 页20: 结束页
        [20, "结尾", "谢谢|全特性防护用例测试完成", "",
         "", "", "", "",
         "", "", "", "",
         "是", ""],

        # 页21: 是否生成=否（测试跳过功能，不应出现在PPT中）
        [21, "内容", "跳过测试页(不应生成)", "1图",
         "column", "按地区汇总", "地区", "总销售额",
         "不应出现的图表", "", "", "透视结果",
         "否", ""],
